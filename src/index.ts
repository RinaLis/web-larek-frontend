...
const basketElement: HTMLTemplateElement = cloneTemplate(
	document.querySelector('#basket') as HTMLTemplateElement
);
const basketContainer = new CardsContainer(
	basketElement.querySelector('.basket__list')
);
const basketView = new Basket(basketElement, basketContainer, events);
const basketCardTemplate: HTMLTemplateElement =
	document.querySelector('#card-basket');

const headerBasket: HTMLButtonElement =
	document.querySelector('.header__basket');
const headerBasketCounter: HTMLElement = headerBasket.querySelector(
	'.header__basket-counter'
);

const deliveryFormTemplate: HTMLTemplateElement =
	document.querySelector('#order');
const deliveryForm = new FormWithButton(
	cloneTemplate<HTMLFormElement>(deliveryFormTemplate),
	events
);
...
events.on('order:changed', () => {
	...
	const cardsArray = basket.products.map((product, number) => {
		const cardInstant = new CardBasket(
