...
Ссылка на проект: https://rinalis.github.io/web-larek-frontend/
...
```
3. Тип отражающий варианты оплаты
```
type TPayment = 'cash' | 'card'
```
4. Данные о доставке товара (соответствуют данным модального окна)
```
export type TDeliveryInfo = Pick<IOrder, 'payment' | 'address'>
```
5. Данные о контактах покупателя (соответствуют данным модального окна)
```
...
#### Класс Component
Абстрактный класс для классов слоя представления. Предоставляет поле `container`, где расположен элемент разметки и метод `render`, возвращающий этот элемент
...
- `cleanOrder(): void;` - очищает полностью корзину
...
- `constructor(data: HTMLElement, events: IEvents)` Конструктор принимает элемент разметки модального окна, экземпляр класса `EventEmitter` для возможности инициации событий

Поля класса
- `modal: HTMLElement;` - элемент модального окна
- `events: IEvents;` - брокер событий
- `content: HTMLDivElement;` - елемент разметки модального окна, куда помещается основной контент модального окна

#### Класс Card
Класс наследуется от абстрактного класса `Component` и объединяет общую функциональность отображения различных форматов карточек товаров на странице. Конструктор принимает элемент темплейта, который нужно заполнить данными и экземпляр класса `EventEmitter` для возможности инициации событий
Методы:
- `render(): HTMLElement;` - метод возвращает полностью заполненную карточку с установленными слушателями
- геттер id возвращает уникальный id карточки
- Также есть геттеры для всех общих элементов карточек: `price` и `title`
...
Методы:
- сеттеры и геттеры для элементов разметки карточки
...
Класс расширяет абстрактный класс `Component`. Отвечает за отображение блоков с карточками. В конструктор принимает HTMLElement контейнера, в котором должны размещаться карточки. \
Предоставляет следующие методы:
- сеттер для добавления содержимого в контейнер
...
- `errorPlace: HTMLElement` - элемент, который используется для вывода ошибок
- `events: IEvents` - экземпляр класса для `EventEmitter` для возможности инициации событий

Методы:
- `set valid(isValid: boolean): void;` - изменяет активность кнопки подтверждения и показывает сообщения об ошибке
...
#### Класс FormWithButton
Класс расширяет функционльность класса `Form`. Добавляется взаимодействие с кнопками формы для выбора одного из вариантов. 

#### Класс BasketView
Класс определяет взаимодействия пользователя с корзиной. Для этого содержит следующие методы
- `setItems(items: HTMLElement[], isOrderCorrect: boolean, price: number): void;` - добавляет элементы в отображение корзины и в зависимости от корректности заказа активирует или инактивирует кнопку `Оформить`
- `render(): void;` - возвращает элемент корзины

#### Класс Success
Класс наследуется от абстрактного класса `Component`. В конструктор принимает HTMLElement разметки окна успешной отправки запроса. Метод `render()`, наследуемый от родителя, возвращает компонент разметки с установленными слушателями и динамичными полями окна
...
Файл `index.ts` представляет собой Presentor, который осуществляет взаимодействие компонентов\
...
- `order:add` - событие генерируется при нажатии на кнопку "Купить"
